...
const CHAR_LIMIT = 40000; // 10 daqiqada bir foydalanuvchi uchun
const usage = new Map();

function checkLimit(userId, chars) {
  const now = Date.now();
  const recent = (usage.get(userId) || []).filter((u) => now - u.at < 10 * 60 * 1000);
  const used = recent.reduce((sum, u) => sum + u.chars, 0);
  if (used + chars > CHAR_LIMIT) throw new HttpError(429, "Juda ko'p ovozli o'qish so'raldi. Birozdan so'ng urinib ko'ring");
  recent.push({ at: now, chars });
...
router.post("/", async (req, res) => {
  ...
  checkLimit(req.user.id, text.length);
