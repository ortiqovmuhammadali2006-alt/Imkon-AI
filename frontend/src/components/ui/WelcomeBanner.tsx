...
export default function WelcomeBanner({
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-600 via-indigo-600 to-violet-700 p-6 text-white shadow-xl shadow-indigo-600/20 sm:p-8">
      ...
      <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-sm font-medium text-indigo-100/90">{todayText()}</p>
          <h1 className="mt-1 text-2xl font-bold tracking-tight sm:text-3xl">
            {greeting()}, {firstName}! 👋
          </h1>
          {subtitle && <p className="mt-2 max-w-xl text-indigo-100/90">{subtitle}</p>}
        </div>
